...
class Ghost
  ...
  def add_letter(letter)
    fragment << letter
    puts "#{current_player} added '#{letter}' to the fragment."
  end
  ...
  def next_player!
    players.rotate!
    players.rotate! until losses[current_player] < MAX_LOSS
  end
  ...
  def update_standings
    ...
    if losses[previous_player] == MAX_LOSS
  ...
  def winner
    (player, _) = losses.find { |_, losses| losses < MAX_LOSS }
    player
  end
end

if __FILE__ == $PROGRAM_NAME
  game = Ghost.new(Player.new("Ray"), Player.new("Peter"))
  game.run
end
